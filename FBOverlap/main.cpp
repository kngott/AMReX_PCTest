...
AMREX_GPU_DEVICE
void compute_bound(int i, int j, int k, int n,
                   Array4<Real> arr, int nw)
{
    Real y = arr(i,j,k,n);
    Real x = 1.0;
    for (int n = 0; n < nw; ++n) {
...
void main_main ()
{
    ...
    int ncell, ncomp, nghost, mgs, nwork=20, ntest=20;
    IntVect lo, hi, bvect;
    {
        ParmParse pp;
        pp.get("lo", lo);
...
// =======================================================
    ...
    // Separate FillBoundary 
    {
        t_fb = amrex::second();

        BL_PROFILE("Separate FB");
        mf.FillBoundary(nghost);
        for (MFIter mfi(mf); mfi.isValid(); ++mfi)
        {
            const Box& bx = mfi.validbox();
            auto arr = mf.array(mfi);
            ...
            [=] AMREX_GPU_DEVICE (int i, int j, int k, int c)
            {
                compute_bound(i, j, k, c, arr, ntest);
            });
        }

        t_fb = amrex::second() - t_fb;
    } 

    // Combine FillBoundary & Pieces
    {
        t_comb = amrex::second();
        BL_PROFILE("Combine FB");

        mf.FillBoundary_nowait(nghost);
        ...
        t_comb = amrex::second() - t_comb;
    }

    amrex::Print() << "FB then Work: " << t_fb << std::endl;
    amrex::Print() << "FB w/ Overlap: " << t_comb << std::endl;
    amrex::Print() << "% Improvement: " << 100*(t_fb-t_comb)/(t_fb) << std::endl;
}
